...
{
    public class CoverCertificate : BaseCertificate<Node>
    {
        public NodeLinkLabel Label { get; }

        public int K { get; }

        public CoverCertificate(SimulationPoint<Node> u, SimulationPoint<Node> v, double t, int k, NodeLinkLabel label) : base(u, v, t)
        {
            K = k;
            Label = label;
            if (label < 0)
        ...
        public override double? GetFailureTime(double CurrentTime)
        {
            ...
            MathNet.Numerics.Polynomial expr = (T * T) - D;
        ...
        {
            return obj is CoverCertificate c && c.K == K && ((c.GetU() == GetU() && c.GetV() == GetV()) || (c.GetU() == GetV() && c.GetV() == GetU())) && Label == c.Label;
        }

        public override int GetHashCode()
        {
            return (Label.GetHashCode() + 2) * (K.GetHashCode() + 1) * (GetU().GetHashCode() * 40 + GetV().GetHashCode() * 60) * 1;
        }*/
