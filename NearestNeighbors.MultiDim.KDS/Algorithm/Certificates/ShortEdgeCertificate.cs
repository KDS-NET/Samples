...
{
    public class ShortEdgeCertificate : BaseCertificate<Node>
    {
        public int K { get; }

        public ShortEdgeCertificate(SimulationPoint<Node> u, SimulationPoint<Node> v, double t, int k) : base(u, v, t)
        {
            K = k; if (k <= 0)
            {
        ...
        {
            return obj is ShortEdgeCertificate c && c.K == K && ((c.GetU() == GetU() && c.GetV() == GetV()) || (c.GetU() == GetV() && c.GetV() == GetU()));
        }

        public override int GetHashCode()
        {
            return (K.GetHashCode() + 1) * (GetU().GetHashCode() * 40 + GetV().GetHashCode() * 60) * 5;
        }*/
