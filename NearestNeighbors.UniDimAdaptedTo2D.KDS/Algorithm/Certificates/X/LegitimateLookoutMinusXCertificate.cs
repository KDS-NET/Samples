...
{
    public class LegitimateLookoutMinusXCertificate : BaseCertificate<Node>
    {
        public LegitimateLookoutMinusXCertificate(SimulationPoint<Node> u, SimulationPoint<Node> v, double t) : base(u, v, t) { }
        ...
        {
            if (GetU().Node.LookoutPointMinusX == null)
            {
                return LessThanCertificate(GetV(), GetU(), Constants.R, CurrentTime);
            }

            double upos = GetU().X.Position;
            double lupos = GetU().Node.LookoutPointMinusX.X.Position;

            double? s = DistancedByLessCertificate(GetU(), GetU().Node.LookoutPointMinusX, 2 * Constants.R, CurrentTime);
            double? s1 = Between1Certificate(GetU().Node.LookoutPointMinusX, GetV(), Constants.R, CurrentTime);
            ...
            if (lupos + Constants.R > upos - Constants.R)
            {
                if (s != null)
                {
                    s1 = Between1Certificate(GetU().Node.LookoutPointMinusX, GetV(), Constants.R, s.Value);
        ...
        public override bool EvaluateValidity(double CurrentTime)
        {
            ...
            {
                double a = GetU().Node.LookoutPointMinusX.X.Position + Constants.R;
