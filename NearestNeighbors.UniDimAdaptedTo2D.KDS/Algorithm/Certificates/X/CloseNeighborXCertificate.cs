...
{
    public class CloseNeighborXCertificate : BaseCertificate<Node>
    {
        public CloseNeighborXCertificate(SimulationPoint<Node> u, SimulationPoint<Node> v, double t) : base(u, v, t) { }

        public override double? GetFailureTime(double CurrentTime)
        {
            MathNet.Numerics.Polynomial expr = (Constants.R * Constants.R) - ((GetV().X.Pol - GetU().X.Pol) * (GetV().X.Pol - GetU().X.Pol));
