...
{
    public class LegitimateLookoutPlusXCertificate : BaseCertificate<Node>
    {
        public LegitimateLookoutPlusXCertificate(SimulationPoint<Node> u, SimulationPoint<Node> v, double t) : base(u, v, t) { }
        ...
        {
            if (GetU().Node.LookoutPointPlusX == null)
            {
                return LessThanCertificate(GetU(), GetV(), Constants.R, CurrentTime);
            }

            double upos = GetU().X.Position;
            double lupos = GetU().Node.LookoutPointPlusX.X.Position;

            double? s = DistancedByLessCertificate(GetU(), GetU().Node.LookoutPointPlusX, 2 * Constants.R, CurrentTime);
            double? s1 = Between1Certificate(GetU(), GetV(), Constants.R, CurrentTime);
            double? s2 = Between2Certificate(GetV(), GetU().Node.LookoutPointPlusX, Constants.R, CurrentTime);
            ...
            if (upos + Constants.R > lupos - Constants.R)
            {
                if (s != null)
                {
                    ...
                    s2 = Between2Certificate(GetV(), GetU().Node.LookoutPointPlusX, Constants.R, s.Value);
                }
                else
                {
                    return null;
        ...
        public override bool EvaluateValidity(double CurrentTime)
        {
            ...
            else
            {
                ...
                double b = GetU().Node.LookoutPointPlusX.X.Position - Constants.R;
                if (a < b)
                {
