...
{
    public class LegitimateLookoutPlusYCertificate : BaseCertificate<Node>
    {
        public LegitimateLookoutPlusYCertificate(SimulationPoint<Node> u, SimulationPoint<Node> v, double t) : base(u, v, t) { }
        ...
        {
            if (GetU().Node.LookoutPointPlusY == null)
            {
                return LessThanCertificate(GetU(), GetV(), Constants.R, CurrentTime);
            }

            double upos = GetU().Y.Position;
            double lupos = GetU().Node.LookoutPointPlusY.Y.Position;

            double? s = DistancedByLessCertificate(GetU(), GetU().Node.LookoutPointPlusY, 2 * Constants.R, CurrentTime);
            double? s1 = Between1Certificate(GetU(), GetV(), Constants.R, CurrentTime);
            double? s2 = Between2Certificate(GetV(), GetU().Node.LookoutPointPlusY, Constants.R, CurrentTime);
            ...
            if (upos + Constants.R > lupos - Constants.R)
            {
                if (s != null)
                {
                    ...
                    s2 = Between2Certificate(GetV(), GetU().Node.LookoutPointPlusY, Constants.R, s.Value);
                }
                else
                {
                    return null;
        ...
        public override bool EvaluateValidity(double CurrentTime)
        {
            ...
            if (GetU().Node.LookoutPointPlusY == null)
            {
                return GetV().Y.Position <= GetU().Y.Position + Constants.R;
            }
            else
            {
                double a = GetU().Y.Position + Constants.R;
                double b = GetU().Node.LookoutPointPlusY.Y.Position - Constants.R;
                if (a < b)
                {
